...
class WeChatSpiderRunner:
    ...
    def scrape_single_account(
            ...
            progress_callback=None, keywords=None  # 接收关键词参数
    ):
        """
        爬取单个公众号（新增关键词筛选功能）
        :param keywords: 关键词列表，用于筛选文章
        """
        logger.info(f"爬取公众号: {name}")
        keywords = keywords or []  # 确保关键词列表默认为空
        ...
        def page_progress_callback(current, total):
            ...
            if progress_callback:
                page_percent = int(current / total * 30)  # 页面爬取占总进度30%
        ...
        logger.info(f"原始获取到 {len(articles)} 篇文章")
        ...
        if start_date and end_date:
            ...
            logger.info(f"过滤日期范围: {start_dt} 至 {end_dt}")
            ...
            for article in articles:
                try:
                    ...
                    logger.warning(f"日期解析失败，跳过文章: {article.get('title')}, 错误: {e}")
        ...
        if keywords:
            ...
            for article in filtered_articles:
                ...
                for kw in keywords:
                    kw_lower = kw.lower()
                    if kw_lower in title or kw_lower in digest:
                        match = True
                        break
            ...
            logger.info(f"关键词筛选后剩余 {len(filtered_articles)} 篇文章 (关键词: {','.join(keywords)})")
        ...
        if include_content or generate_pdf:
            ...
            if total_articles == 0:
                ...
                if progress_callback:
                    progress_callback(100, "无符合条件的文章")
            ...
            for i, article in enumerate(filtered_articles):
                ...
                # 触发进度回调（总进度 = 页面30% + 内容处理70%）
                if progress_callback:
                    content_percent = int(30 + (i + 1) / total_articles * 70)
                    progress_callback(content_percent,
        ...
        if use_db and filtered_articles:
            ...
            try:
                ...
                return {"success": False, "msg": f"数据库错误: {str(e)}", "data": {}}

        if success:
            logger.success(f"成功保存 {len(filtered_articles)} 篇文章")
            return {
                "success": True,
                "msg": f"爬取完成，共获取 {len(filtered_articles)} 篇文章",
                "data": {"count": len(filtered_articles), "path": output_path}
            }
        else:
            logger.error("保存失败")
            return {"success": False, "msg": "保存结果失败", "data": {}}
    ...
    def batch_scrape(
        ...
        def batch_completed_callback(total_articles):
            logger.success(f"批量爬取完成，原始获取 {total_articles} 篇文章")
        ...
        if keywords:
            ...
            logger.info(f"关键词筛选后剩余 {len(articles)} 篇文章 (关键词: {','.join(keywords)})")
        ...
        if generate_pdf and articles and batch_pdf_dir:
            ...
            for i, article in enumerate(articles):
                if progress_callback:
                    pdf_percent = int(40 + (i + 1) / total_articles * 60)  # 公众号爬取40% + PDF生成60%
        ...
        return {
            "success": True,
            "msg": f"批量爬取完成，共获取 {len(articles)} 篇文章",
            "data": {"count": len(articles), "path": config['output_file']}
        }
